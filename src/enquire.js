// enquire.js v1.0 - awesome media queries in javascript
// (c) Nick Williams https://github.com/WickyNilliams
...
window.enquire = (function(matchMedia) {
    ...
    function QueryHandler(options) {
        ...
        if(!options.deferSetup) this.setup();
    }
    ...
    MediaQuery.prototype = {
         ...
         */
        match : function(e) {
            if(this.matched) return; //already on
         ...
         */
        unmatch : function(e) {
            if(!this.matched) return; //already off

            each(this.handlers, function(handler){
                handler.off && handler.off(e);
            });
            this.matched = false;
        }
    ...
    function MediaQueryDispatch () {
        if(!matchMedia) {
            throw Error("matchMedia is required for media query dispatcher");
        }
    ...
    MediaQueryDispatch.prototype = {
        ...
        fire : function(e) {
            ...
            for(mediaQuery in queries) {
                if(!queries.hasOwnProperty(mediaQuery)) continue;

                var mq = queries[mediaQuery];
                mq.matchMedia() ? mq.match(e) : mq.unmatch(e);
            }
            return this;
        },
